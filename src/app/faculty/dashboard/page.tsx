...
import { ShieldCheck, Users, FileCheck, AlertCircle, ArrowUpRight, Layers, Download, X, FileText, Award, RefreshCcw } from "lucide-react";
...
export default function FacultyDashboard() {
  ...
  const handleManualRefresh = async () => {
    ...
    await refresh();
    // Ensure the spin is visible for at least a heartbeat
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between mb-10 md:mb-16 gap-6 md:gap-8">
          ...
          <div className="flex flex-col sm:flex-row items-center gap-4 shrink-0">
            <button 
              ...
              className="w-12 h-12 bg-bg-surface border-3 border-bg-dark rounded-2xl flex items-center justify-center text-text-primary hover:bg-accent hover:shadow-[4px_4px_0_#000] transition-all active:translate-x-[2px] active:translate-y-[2px] active:shadow-none mr-2 group"
              title="Pulse Refresh (Every 5s)"
            >
              <motion.div
                animate={isRefreshing ? { rotate: 360 } : { rotate: 0 }}
                transition={isRefreshing ? { duration: 1, repeat: Infinity, ease: "linear" } : { duration: 0.5 }}
              >
      ...
      >
        {loading && certificates.length === 0 ? (
          Array.from({ length: 3 }).map((_, i) => (
            <div key={i} className="bento-card h-48 bg-bg-surface border-4 border-border animate-pulse rounded-[2rem] flex flex-col justify-between p-8">
