...
export default function FacultySectionsPage() {
  const { user } = useAuth();
  const { certificates, updateStatus, loading: certsLoading } = useCertificates();
  ...
  const handleExport = (format: "CSV" | "Excel") => {
    ...
    const dataToExport = filteredStudents.map(s => {
      const certs = getStudentCerts(s.rollNo);
      ...
      return {
        ...
        "Top Category": verified.length > 0 ? (totalScore / verified.length > 90 ? "Platinum" : "Gold") : "N/A"
      };
    });
    ...
    <DashboardLayout allowedRole="faculty">
      <div className="space-y-12">
        <div>
          ...
          <p className="text-text-secondary text-sm font-bold uppercase tracking-widest opacity-60">Manage student credentials across academic clusters.</p>
        </div>
