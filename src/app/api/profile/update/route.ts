...
export async function POST(req: NextRequest) {
  try {
    ...
    if (reg_no && !REG_REGEX.test(reg_no.trim().toUpperCase())) {
      return NextResponse.json(
        { error: "Invalid registration number format. Expected: AU/2023/0008918" },
        { status: 400 }
      );
    }
    ...
    if (profile.role === "student") {
      ...
      if (sections_managed !== undefined) {
        updatePayload.sections_managed = Array.isArray(sections_managed) ? sections_managed : [];
      }
      if (batches_managed !== undefined) {
        updatePayload.batches_managed = Array.isArray(batches_managed) ? batches_managed : [];
      }
    ...
    if (updateError || !updated) {
      return NextResponse.json(
        { error: "Update failed — concurrent edit detected or profile not found." },
        { status: 409 }
      );
    }

    // 6. Sync display name to Clerk (server-side only)
